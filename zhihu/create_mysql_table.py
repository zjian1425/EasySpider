...
def operate_mysql():
    ...
    conn = pymysql.connect("localhost","username","passwd","db_name",charset = "utf8")

    cursor = conn.cursor()
    do_create_users_info = """CREATE TABLE users_info_test (
    ...
    cursor.execute("drop table if exists users_info_test ")
    ...
    do_create_url_token = """create table url_token_test (id int auto_increment,
                              url_token varchar(20) primary key,key(id))"""

    cursor.execute("drop table if exists url_token_test ")
    #实现id自增且不是主键
